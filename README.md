...
Wild Web Developer supports contributing XML-LS extensions through it's **xmllsExtension extension point**. To use the extension point, create an xmllsExtension extension in an Eclipse plug-in project and set the path to the location of the maven extension jar (which must reside within the plug-in project).
...
<extension
    ...
    <jar
        path="org.eclipse.lsp4xml.extensions.maven-0.9.1.jar">
    </jar>
</extension>
```

The jar can be built by cloning this repository and running `mvn clean verify`; the resulting jar will be located in the `target` folder.

Alternatively, the prebuilt jar can be downloaded from [here](https://github.com/angelozerr/vscode-xml-maven/tree/master/jar).
